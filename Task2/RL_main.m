...
test_gammas = [0.9]; % gamma the discount factor
    ...
    for decay_meth=[2]    
        ...
        parfor r = 1:Total_runs 
            [Q_vals(:,:,r), ~, timer(r), suc_runs(r)] = ...         
                        Q_Trials(qevalreward, disc_fac, decay_meth, ...      % caculate the Q values 
                        decay_meth, threshold, false);                  
            optim_pol(:,r) = cal_Optim_Pol(Q_vals(:,:,r));              % Optimal policy takes actions with max Q values
            Final_Reward(r) =  cal_reward(optim_pol(:,r), qevalreward);      % calculate total reward for 100 states 
        end
        ...
        qevalstates = plott(optim_pol(:,max(goal_ReachedRuns)));                  % plot trajectory optimal goal reached run
